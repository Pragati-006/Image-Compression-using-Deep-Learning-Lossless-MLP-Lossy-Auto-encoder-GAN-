...
def apply_classification_loss_mse(kernels1=[5, 7], kernels2=[7, 5],
    ...
    with tf.Graph().as_default() as g:
        with tf.device("/gpu:0"):  # use gpu:0 if on GPU
            ...
            (x_out, x_out_quant) = cnn_autoencoder(input_layer, pool_size=pool_size, kernels1=kernels1, filters1=filters1,
                                                   kernels2=kernels2, filters2=filters2)

            mse_loss1 = tf.reduce_mean(tf.subtract(input_layer, x_out) ** 2)
            mse_loss2 = tf.reduce_mean(tf.subtract(input_layer, x_out_quant) ** 2)
    ...
    model_dict = {'graph': g, 'inputs': input_layer, 'outputs': x_out, 'train_op': train_op, 'loss1': mse_loss1,
                  'loss2': mse_loss2}
